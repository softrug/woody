...
// User: Hi
// Agent: Hello, I'm Woody, your AI trading assistant. Which blockchain network would you like to explore?
// User: Let's do pepe
// Agent: Great choice, let's look into pepe.
// ...
...
These are the available chains to consider:
{{availableChains}}

These are the recent messages exchanged:
{{recentMessages}}

Based on the recent messages and the available options, identify the chain. Utilize only the available chains; if none are mentioned, return null.

Always prioritize the chain from the latest messages, followed by older ones. If the user recently expressed interest in switching to a chain, select that chain.
